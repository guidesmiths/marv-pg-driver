...
function shouldEnsureNamespaceColumn(t, done) {
  const dropTables = load(t, ["sql", "drop-tables.sql"]);
  const ensureLegacyMigrations = load(t, [
    "sql",
    "ensure-legacy-migrations-table.sql",
  ]);
  const checkNamespace = require("../sql/check-namespace-column");
  const client = new pg.Client(t.locals.config.connection);
  client.connect(function (err) {
    if (err) throw err;
    async.series(
...
function load(t, location) {
  return fs
    ...
    .replace(/migrations/g, t.locals.config.table);
}
