...
function shouldEnsureNamespaceColumn(t, done) {
  ...
  client.connect(function (err) {
    if (err) throw err;
    async.series([client.query.bind(client, dropTables), client.query.bind(client, ensureLegacyMigrations)], function (err) {
      if (err) throw err;
      marv.scan(path.join(__dirname, 'migrations'), function (err, migrations) {
        if (err) throw err;
...
module.exports = Hath.suite('Driver Tests', [shouldRunMigration, shouldEnsureNamespaceColumn]);
